...
\AddEverypageHook{%
  \begin{picture}(0,0)
    \put(-42,-420){\rotatebox{90}{\color{gray!60}\fontsize{28}{32}\selectfont\textsc{Final Report}}}
  \end{picture}%
}
...
This combination led to very good coverage of the first level of the game and the character trajectories were also very varied. The first attempt did not turn out to be and optimal mix. The \texttt{SIMPLE\_MOVEMENT} control mechanism provided by the emulator does allow for movement \texttt{left} and \texttt{right}, but only has combinations with the \texttt{A} and \texttt{B} keys for the \texttt{right}, so 4 of the possible 7 actions move the game character forward. This was also exacerbated by the AI strategy, which used these actions even more than the uniform random distribution. This resulted in our original training dataset of 665 816 images only containing 5,42\% \texttt{left} actions. As a consequence of these imbalances, our first round of models continued moving the character forward even without user input.

In our enhanced dataset, this was increased to 17,05\% while also increasing the number of frames where no action was taken from 14,81\% to 50,05\%. The current strategy relies on running the AI strategy for an amount of time from a uniform distribution of the level runthrough, but only starting recording data once the strategy is switched to fully random. This results in recorded runs starting at random places along the optimal path through the level (see black dots in Figure~\ref{fig:collected_data3_heatmap}) and being guided entirely by a random uniform distribution across the directionally balanced \texttt{COMPLEX\_MOVEMENT}\footnote{The \texttt{up} and \texttt{down} actions were not used as they are only useful when climbing latters.}. This dataset was better, but the control strategy was still to "twitchy" and didn't hold keys for as long as a user would. There was also a new issue when Mario became stuck (not moving) by going left and getting stuck between the left edge of the screen and the staircase like structures, which is a difficult situation to climb out of randomly.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/collected_data3_all_runs_heatmap_562k.png}
  \caption{Third-generation dataset: occupancy heatmap over World 1--1 aggregated across all runs (561 563 logged frames).}
  \label{fig:collected_data3_heatmap}
\end{figure*}

Our third generation dataset improved on this drastically by using more shorter runs. Using the coordinate metadata extracted from the simulator, Mario now moves as expected with each action, see relative frame-to-frame coordinates heatmap in Figure \ref{fig:collected_data3_actions}.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/collected_data3_actions_562k.png}
  \caption{Third-generation dataset: direction maps, showing the probability of per-frame position change. The count of each action in the dataset can also be seen. Notice they are now pretty uniform.}
  \label{fig:collected_data3_actions}
\end{figure*}

Further discussion of the impact the improved dataset had on model performance can be found in Section \ref{eval} and Figure \ref{fig:data-compare}.

\section{GameNGen architecture}
\label{gamengen}

GameNGen\footnote{\url{https://arxiv.org/abs/2408.14837}} is an architecture originally developed for simulating the video game DOOM. It is based on a pre-trained text-to-image model, speciffically Stable Diffusion 1.4. This model is used to predict the next frame given the sequence of previous actions (player inputs) and observations (frames). Each action from the sequence is encoded into a single token, which are then used to replace the cross attention from text encoding generally used by Stable Diffusion. The sequence of frames is encoded using the autoencoder and then concatenated to the noised latents in the latent channels dimension. This produces a tensor, where the number of channels is 4*n, where n is the total number of input frames. Since the Stable diffusion unet expects only 4 dimension, the first convolutional layer is replaced by a randomly initialized layer with the correct input channel amount. This layer catches up very quickly during training and doesn't pose further problems.

Since the code for the official GameNGen implementation is not public, we have used an unofficial reproduction\footnote{\url{https://github.com/arnaudstiegler/gameNgen-repro}}. This implementation has reached similar results in regards to visual quality, however it doesn't reach 20 FPS as reported in the original paper. We have managed to offset this by reducing the diffusion iteration count and using more powerful hardware.
...
We have managed to produce a model that has good visual fidelity when compared to original data. By tuning the training dataset we have also achieved a decent amount of responsivity to the player inputs. This is further explored in the evaluation chapter.
...
\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/controlnet_based.png}
  \caption{Qualitative evaluation of the Transformer's spatial prediction during the training phase. From left to right: (1) The ground truth RGB game frame, (2) the corresponding ground truth multi-class segmentation mask, and (3) the predicted segmentation mask generated by our Transformer-decoder architecture. The predicted mask serves as the intermediate structural representation that will subsequently condition the ControlNet rendering.}
  \label{fig:controlnet_based}
\end{figure*}
...
An interactive version of our ControlNet based approach is also available.

\section{Evaluation} \label{eval}

While we discussed PSNR previously, we are currently focusing on less pixel-level, more perceptual metrics. One of these is LPIPS (Learned Perceptual Image Patch Similarity) \footnote{\url{https://arxiv.org/abs/1801.03924}}, which is also cited in the GameNGen paper. This metric is also available from a PyPI package from the authors of the paper\footnote{\url{https://pypi.org/project/lpips/\#1-learned-perceptual-image-patch-similarity-lpips-metric}}. We have employed both PSNR and LPIPS in an evaluation script.

The plot displayed in Figure \ref{fig:real-gameplay-eval} is the output of the script. The ground truth data is an instrumented capture of human gameplay, compared with the output of our best model. A side by side GIF of the ground truth and generated frames is available at this permalink to GitHub \url{https://github.com/MichaelPestuka/KNN_proj/tree/80cf16b67242582644d0ae407c9e8687707fa741/figures/GIFs/sd-mario-3}.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/sd-mario-3_human_data.png}
  \caption{An evaluation of per-frame metrics on a few runs from the real user gameplay data}
  \label{fig:real-gameplay-eval}
\end{figure*}

Across this evaluation set of 10 gameplay sequences, the model at 3 diffusion steps achieves a better LPIPS result than the original GameNGen at 4 diffusion steps, but the given the different gameplay of Doom such a comparison may not be so fair.

Our latest model \texttt{sd-mario-3} is also a big improvement compared to our first try presented at the checkpoint (\texttt{sd-fixed}), this comparison can be seen in Figure \ref{fig:data-compare}. The difference between the datasets these models were trained on is further described in Section \ref{data-generation}.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/data_compare_eval_1&3.png}
  \caption{An evaluation of aggregated per-frame metrics on a few runs from the real user gameplay data across our original model presented at checkpoint (\texttt{sd-fixed}) and our latest (\texttt{sd-mario-3})}
  \label{fig:data-compare}
\end{figure*}

We also compared different context length models on our original dataset, the results of this comparison can be seen in Figure \ref{fig:context-compare}. The results are most favourable for the 9 frame context, which was also used in the original GameNGen paper.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/context_length_compare_human_data.png}
  \caption{An evaluation of aggregated per-frame metrics on a few runs from the real user gameplay data across varying model context lengths. Each model was trained separately for that context length.}
  \label{fig:context-compare}
\end{figure*}

We hope to present a playable demo at the poster session to allow for hands on evaluation.
