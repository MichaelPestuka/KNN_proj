...
We have employed both PSNR and LPIPS in an evaluation script, but currently our models generate images outside the scope of this kind of evaluation, failing to model primary game mechanics on unseen data.

You can see an example per frame evaluation in figure \ref{fig:basic_eval}. A side by side GIF of the ground truth and generated frames from the best and worst example shown (training example \texttt{0aa9351d} and \texttt{0aaa90ef} respectively) can be viewed at \url{https://github.com/MichaelPestuka/KNN\_proj/tree/be189cbf683eac64d27bfda323cdc9953e7e1646/figures/GIFs}.

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/batch_eval_first_gen_data_frame_metrics.png}
  \caption{An evaluation of per-frame metrics on a few runs from the training set. Not reprenstative of generalization performance, but a demonstration of our working evaluation. On unseen data performance graphs just show random noise, for now.}
  \label{fig:basic_eval}
\end{figure*}

We have also relied heavily on the interactive mode during development to better understand model behaviour.

\section{Future work}

Currently we are focusing on re-running our training with the updated dataset. We hope this will mitigate the controllability issues with the GameNGen implementation. After these issues are resolved, we plan to focus on optimizing the model and finding the parameters (diffusion inference steps, context frame count) with the best results.
...
While the current implementation of the ControlNet based approach validates the viability of decoupling state prediction from visual rendering, a primary focus of our future work will be experimentation with training strategy and hyperparameter tuning. Specifically, we will explore variations in the autoregressive rollout schedules, adjusting the sequence length and the progressive phase of longer horizons to better mitigate accumulated drift over extended gameplay episodes. On the generative side, experiments will be conducted to find the optimal trade-off between visual fidelity and real-time inference latency by tuning the Stable Diffusion inference parameters. Ultimately, the optimized pipeline will be evaluated using the previously established metrics and integrated into the interactive environment discussed earlier. This crucial final step will allow us to empirically verify the actual playability, responsiveness, and overall robustness of the fully simulated game.
