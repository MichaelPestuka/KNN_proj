...
def preprocess_train(examples):
    ...
    return {"pixel_values": images, "input_ids": actions, "episode_ids" : examples["episode_id"]}
...
class EpisodeDataset:
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        padding_len = 0
        for i in range(idx-BUFFER_SIZE, idx):
            if(self.dataset[i]["episode_ids"] != self.dataset[idx]["episode_ids"]):
                padding_len += 1

        if(padding_len > 0):
            padding = [IMG_TRANSFORMS(Image.new('RGB', (WIDTH, HEIGHT), color='black')) for _ in range(padding_len)]
            return {'pixel_values': padding + self.dataset[idx+padding_len-BUFFER_SIZE:idx+1]['pixel_values'], 'input_ids': torch.concat([torch.zeros(len(padding), dtype=torch.long), self.dataset[idx+padding_len-BUFFER_SIZE:idx+1]['input_ids']])}
