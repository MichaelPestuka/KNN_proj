#Info
This file contains information on how to run gameNgen training followed by the original repo README
...
to the directory with training scripts.

Then after activating the environment, simply running this command should be enough:
accelerate launch train_text_to_image.py --dataset_name Michalp03/mario-3 --gradient_checkpointing --learning_rate 5e-5 --train_batch_size 32 --dataloader_num_workers 18 --num_train_epochs 4 --validation_steps 100 --use_cfg --output_dir sd-mario --lr_scheduler cosine --report_to wandb

The dataset will be automatically downloaded from my huggingface.

note: batch size may need to be tuned depending on available VRAM.
