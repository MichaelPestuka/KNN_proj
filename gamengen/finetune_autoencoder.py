...
def eval_model(model: AutoencoderKL, test_loader: DataLoader) -> float:
    ...
    with torch.no_grad():
        ...
        for batch in progress_bar:
            ...
            recon = model(data).sample.clamp(0.0, 1.0)
...
def main():
    ...
    # Dataset Setup
    dataset = load_dataset("Michalp03/mario_big_better")
    split_dataset = dataset["train"].train_test_split(test_size=50, seed=42)
    ...
    model.save_pretrained(os.path.join(args.output_dir, "vae"))
