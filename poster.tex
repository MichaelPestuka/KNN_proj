...
\begin{column}{0.47\textwidth}
  ...
  \begin{block}{Data Generation}
    Training data was collected from a \textbf{Super Mario Bros.\ emulator}\cite{gymsmb}
    using varying strategies, including a pre-trained AI system\cite{yumouwei2023rl} playing the game. This results in good level coverage (see full-width occupancy heatmap at the bottom of this poster).

    \smallskip
    Early datasets were heavily biased toward \emph{right-moving} actions
    ($<$6\,\% left) and the resulting model suffered from ignoring user input. The 3rd-generation dataset fixed this with a balanced ratio of actions and taken while also including frames where movement actually happened and the character isn't stuck.
  ...
  \begin{block}{\centering \textbf{GameNGen Architecture}}

\setlength{\parskip}{0.4em}

\textbf{GameNGen} builds on a pretrained \textbf{Stable Diffusion 1.4} model to predict the next game frame conditioned on:
\begin{itemize}
    \item a history of $n$ previous frames, and
    \item a history of $n$ previous and the current player actions.
\end{itemize}

\vspace{0.3em}

\textbf{Input adaptation.}
The first convolutional layer of the Stable Diffusion U-Net is replaced with a newly initialized layer that accepts the latent representations of the $n$ history frames concatenated along the channel dimension.  
This increases the input channel count from $4$ to $4n$.

\vspace{0.3em}

\textbf{Action conditioning.}
Player actions are transformed into learned embeddings and injected into the model through cross-attention conditioning.

\vspace{0.3em}

\textbf{Decoder fine-tuning.}
To improve visual sharpness at low resolution, the decoder is additionally fine-tuned on the gameplay dataset.

\vspace{0.3em}

\textbf{Autoregressive generation.}
Frames are generated autoregressively, where the diffusion sampling step count is tuned to maintain a stable real-time framerate.

\vspace{0.5em}

{\footnotesize
Implementation based on GameNGen~\cite{valevski2025diffusionmodelsrealtimegame} and the open-source reproduction by Arnaud Stiegler~\cite{stiegler2024gamengenrepro}.
}
...
\begin{column}{0.47\textwidth}
  ...
  \begin{block}{ControlNet-Based Approach}
    ...
    \smallskip
    \textbf{3. ControlNet Renderer} --- Stable Diffusion + ControlNet
    ...
    \begin{itemize}\setlength\itemsep{1pt}
      \item \emph{Progressive rollout}: Scales from 1-step teacher forcing up to 100-step autoregressive sequences to combat error drift.
      \item \emph{Composite loss}: Standard BCE combined with soft Dice loss (penalises errors on sparse objects like Mario) and Sobel boundary loss (enforces crisp edges).
    \end{itemize}
  \end{block}
  ...
  \begin{block}{Evaluation}
    ...
    \begin{center}
      ...
      \par\vskip2pt
      {\footnotesize\color{captioncolor}%
        \texttt{sd-fixed} (presented at checkpoint) vs.\ \texttt{sd-mario-3} (latest model) ---
    ...
    \begin{tcolorbox}[colback=gray!7, colframe=black, sharp corners,
      ...
      \textbf{better LPIPS than original GameNGen at 4 steps}. This game is
      less complex, and methodology differences may also play a role.
    \end{tcolorbox}
  \end{block}
  ...
  \begin{block}{References}
    \renewcommand*{\bibfont}{\rmfamily\scriptsize}
...
\vfill
\vskip0.3cm
{\rmfamily\bfseries\normalsize World 1-1 Coverage Heatmap}
...
\begin{center}
  ...
  \par\vskip2pt
  {\footnotesize\color{captioncolor}%
    3rd-gen dataset: character position heatmap aggregated across all runs
    (561\,563 logged frames). Black and white dots show recorded run start and end positions respectively.}
\end{center}
